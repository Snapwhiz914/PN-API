...
import { Box, Group, Stack, TextInput, Slider, MultiSelect, Button, Text, Paper, Loader, Badge, Flex, ActionIcon, Tooltip, Modal, } from '@mantine/core';
...
function getIconForSpeed(speed) {
    if (speed <= 1000)
        return speedIcons.red;
    if (speed <= 2000)
        return speedIcons.violet;
    if (speed <= 3000)
        return speedIcons.orange;
    if (speed <= 4000)
        return speedIcons.gold;
    if (speed <= 5000)
        return speedIcons.yellow;
    if (speed <= 6000)
        return speedIcons.green;
    if (speed <= 7000)
        return speedIcons.blue;
    if (speed <= 8500)
        return speedIcons.grey;
...
const sliderToMinutes = (sliderValue) => {
    if (sliderValue === 0)
        return 1;
...
const markerVisibility = new Map();
export function MapPage({ onNavigateHome }) {
    const mapContainer = useRef(null);
    const map = useRef(null);
    const markerClusterGroup = useRef(null);
    const markers = useRef(new Map());
    const clusters = useRef(new Map());
        ...
        markerClusterGroup.current = L.markerClusterGroup({
            ...
            iconCreateFunction: (cluster) => {
                const visibleCount = cluster
                    .getAllChildMarkers()
                    .filter((m) => markerVisibility.get(m._leaflet_id) !== false)
                    .length;
                return L.divIcon({
    ...
    useEffect(() => {
        if (!map.current || !markerClusterGroup.current)
            return;
        // Add/update markers
    ...
    const updateStats = () => {
        ...
        const anonymityCounts = { '-1': 0, '0': 0, '1': 0, '2': 0, '3': 0 };
        let totalSpeed = 0;
        for (const proxy of proxies) {
    ...
    const generatePopupContent = (proxy) => {
        const lastCheckDate = new Date(proxy.last_check);
        const now = new Date();
    ...
    return (_jsxs(Box, { style: {
            ...
            height: '100vh',
            overflow: 'hidden',
        }, children: [_jsx(Paper, { p: "md", style: { flex: '0 0 auto', overflowY: 'visible', maxHeight: 'none' }, children: _jsxs(Stack, { gap: "sm", style: { overflowY: 'auto', maxHeight: '30vh' }, children: [_jsxs(Group, { justify: "space-between", children: [_jsx(Text, { fw: 500, size: "lg", children: "Proxy Map Filters" }), onNavigateHome && (_jsx(Button, { variant: "subtle", size: "xs", onClick: onNavigateHome, children: "Back to Profiles" }))] }), _jsxs(Group, { gap: "md", wrap: "wrap", style: { width: '100%' }, children: [_jsx("div", { style: { minWidth: '250px', maxWidth: '300px' }, children: _jsx(MultiSelect, { label: "Countries", placeholder: "Select countries", data: allCountries.map((c) => ({ value: c, label: c })), value: filters.countries, onChange: (value) => {
                                            handleFilterChange({ countries: value });
                                            setFilters((prev) => ({ ...prev, countries: value }));
                                            fetchProxies();
                                        }, searchable: true, clearable: true, styles: { dropdown: { zIndex: 1000 } } }) }), _jsx("div", { style: { minWidth: '180px', maxWidth: '220px' }, children: _jsx(TextInput, { label: "Region", placeholder: "e.g., California", value: filters.region, onChange: (e) => {
                                            handleFilterChange({ region: e.currentTarget.value });
                                            updateVisibility();
                                        } }) }), _jsx("div", { style: { minWidth: '180px', maxWidth: '220px' }, children: _jsx(TextInput, { label: "City", placeholder: "e.g., New York", value: filters.city, onChange: (e) => {
                                            handleFilterChange({ city: e.currentTarget.value });
                                            updateVisibility();
                                        } }) }), _jsxs("div", { style: { minWidth: '220px', maxWidth: '280px' }, children: [_jsxs(Text, { size: "sm", fw: 500, mb: 8, children: ["Max Speed: ", filters.speed, "ms"] }), _jsx(Slider, { min: 20, max: 10000, step: 100, value: filters.speed, onChange: (value) => {
                                                handleFilterChange({ speed: value });
                                                updateVisibility();
                                            } })] }), _jsxs("div", { style: { minWidth: '220px', maxWidth: '280px' }, children: [_jsxs(Text, { size: "sm", fw: 500, mb: 8, children: ["Checked since: ", Math.floor(sliderToMinutes(lastCheckSlider) / 60), "h ", sliderToMinutes(lastCheckSlider) % 60, "m"] }), _jsx(Slider, { min: 0, max: 100, step: 1, value: lastCheckSlider, onChange: (value) => {
                                                setLastCheckSlider(value);
                                                const minutes = sliderToMinutes(value);
                                                handleFilterChange({ lastCheck: minutes });
                                                updateVisibility();
                                            } })] }), _jsx("div", { style: { minWidth: '220px', maxWidth: '280px' }, children: _jsx(MultiSelect, { label: "Protocols", placeholder: "Select protocols", data: PROTOCOL_OPTIONS, value: filters.protocols, onChange: (value) => {
                                            handleFilterChange({ protocols: value });
                                            updateVisibility();
                                        }, searchable: true, clearable: true, styles: { dropdown: { zIndex: 1000 } } }) }), _jsx("div", { style: { minWidth: '220px', maxWidth: '280px' }, children: _jsx(MultiSelect, { label: "Anonymity", placeholder: "Select anonymity levels", data: ANONYMITY_OPTIONS, value: filters.anonymity, onChange: (value) => {
                                            handleFilterChange({ anonymity: value });
                                            updateVisibility();
                                        }, searchable: true, clearable: true, styles: { dropdown: { zIndex: 1000 } } }) }), _jsxs(Flex, { gap: "sm", align: "flex-end", children: [_jsx(Button, { onClick: () => {
                                                setFilters({
                                                    countries: [],
                                                    region: '',
                                                    city: '',
                                                ...
                                                setLastCheckSlider(minutesToSlider(720));
                                                fetchProxies();
                                            }, children: "Reset Filters" }), _jsx(Tooltip, { label: "Refresh proxies", children: _jsx(ActionIcon, { onClick: fetchProxies, loading: loading, variant: "default", size: "lg", children: _jsx(IconRefresh, { size: 16 }) }) })] })] })] }) }), _jsx(Box, { ref: mapContainer, style: {
                    flex: '1 1 auto',
                    position: 'relative',
                } }), _jsx(Paper, { p: "md", style: { flex: '0 0 auto' }, children: _jsxs(Group, { justify: "space-between", children: [_jsxs(Group, { children: [_jsx("div", { children: _jsxs(Text, { size: "sm", children: ["Total proxies: ", stats.totalProxies, " | Shown: ", _jsx(Badge, { children: stats.shownProxies })] }) }), loading ? (_jsx(Loader, { size: "sm" })) : (_jsx(Text, { size: "sm", c: error ? 'red' : 'green', children: error || 'Fetch Success!' }))] }), _jsxs(Group, { children: [_jsx("div", { children: _jsxs("label", { children: [_jsx("input", { type: "checkbox", checked: pauseRefresh, onChange: (e) => setPauseRefresh(e.currentTarget.checked) }), ' Pause Refresh'] }) }), _jsx(Text, { size: "sm", children: refetchCountdown > 0 ? `Next fetch: ${refetchCountdown}s` : 'Fetching...' }), _jsx(Button, { size: "xs", variant: "subtle", onClick: () => setStatsModalOpen(true), children: "Stats" })] })] }) }), _jsx(Modal, { opened: statsModalOpen, onClose: () => setStatsModalOpen(false), title: "Proxy Statistics", size: "sm", children: _jsxs(Stack, { gap: "md", children: [_jsxs("div", { children: [_jsx(Text, { fw: 500, size: "sm", children: "Protocols:" }), _jsxs(Text, { size: "sm", children: ["HTTP: ", stats.protocolCounts['http'], ", HTTPS: ", stats.protocolCounts['https'], ", SOCKS4:", ' ', stats.protocolCounts['socks4'], ", SOCKS5: ", stats.protocolCounts['socks5'], ", Unknown:", ' ', stats.protocolCounts['unknown']] })] }), _jsxs("div", { children: [_jsx(Text, { fw: 500, size: "sm", children: "Anonymity:" }), _jsxs(Text, { size: "sm", children: ["Unknown: ", stats.anonymityCounts['-1'], ", None: ", stats.anonymityCounts['0'], ", Low:", ' ', stats.anonymityCounts['1'], ", Medium: ", stats.anonymityCounts['2'], ", High:", ' ', stats.anonymityCounts['3']] })] }), _jsx("div", { children: _jsxs(Text, { fw: 500, size: "sm", children: ["Average Speed: ", stats.avgSpeed, "ms"] }) })] }) })] }));
}
